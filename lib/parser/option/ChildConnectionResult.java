...
public enum ChildConnectionResult {
    /**
     * 成功
     */
    SUCCESS(0x00),
     ...
     */
    ChildConnectionResult(int resultCode) {
     ...
     */
    public static ChildConnectionResult getEnumByResultCode(int resultCode) {
        ChildConnectionResult[] enumArray = ChildConnectionResult.values();

        for (ChildConnectionResult enumInt : enumArray) {
            if (resultCode == enumInt.resultCode) {
                return enumInt;
            }
        }

        throw new IllegalArgumentException("not found resultCode: " + resultCode);
    }
}
