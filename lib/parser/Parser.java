...
public class Parser {
    ...
    public Result parse(int data) {
        ...
        // チェックサムを受信するまで待つ
        int dataLength = (int) Util.mergeBigEndian(Arrays.copyOfRange( //
                this.receiveData, //
        ...
        if (checksum == calculateChecksum(this.receiveData, this.receiveDataLength - 1)) {
     ...
     * @return 計算したチェックサム値
     */
    int calculateChecksum(int[] arr, int length) {
        int checksum = 0;
