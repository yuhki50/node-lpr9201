    ...
    public Map<Integer, Integer> getRssis() {
        ...
        if (length == RATE1_LENGTH) {
            for (int i = 0; i < length; i++) {
                rssis.put(START_CH + i, this.result.datas[i] | ~0xFF);
            }
        } else if (length == RATE2_LENGTH) {
            int offset = 0;
            for (int i = 0; i < length; i++) {
                rssis.put(START_CH + offset + i, this.result.datas[i] | ~0xFF);
                rssis.put(START_CH + offset + i + 1, this.result.datas[i] | ~0xFF);
                offset++;
            }
        } else {
