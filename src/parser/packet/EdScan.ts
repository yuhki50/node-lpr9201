...
class EdScan implements IParsePacket {
     ...
     * 結果コード
     */
    //protected static const RESULT_CODE : number = 0x8C;
    protected static RESULT_CODE : number = 0x8C;
     ...
     * Rate1のチャンネル数
     */
    //private static const RATE1_LENGTH : number = 0x1D;
    private static RATE1_LENGTH : number = 0x1D;

    /**
     * Rate2のチャンネル数
     */
    //private static cons RATE2_LENGTH : number = 0x0E;
    private static RATE2_LENGTH : number = 0x0E;

    /**
     * 開始チャンネル番号
     */
    //private static const START_CH : number = 33;
    ...
    public getRssis() : {[key : number] : number} {
        ...
        if (length == EdScan.RATE1_LENGTH) {
            for (var i = 0; i < length; i++) {
                rssis[i] = EdScan.START_CH + i, this.result.datas[i] | ~0xFF;
            }
        } else if (length == EdScan.RATE2_LENGTH) {
            var offset = 0;
            for (var i = 0; i < length; i++) {
                rssis[i] = EdScan.START_CH + offset + i, this.result.datas[i] | ~0xFF;
                rssis[i] = EdScan.START_CH + offset + i + 1, this.result.datas[i] | ~0xFF;
                offset++;
            }
        } else {
