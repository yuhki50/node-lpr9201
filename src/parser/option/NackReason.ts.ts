...
class _NackReason {
    protected static REASON_CODE_MAP:number[] = [
        0x01,  // PARAMETER_ERROR
        0x02,  // SEND_RETRY_FAILURE
        0x03,  // NO_RECEIVE_DATA
        0x04,  // NETWORK_ERROR
        0x05,  // UART_OVERRUN_ERROR
        0x06,  // UART_BYTE_INTERVAL_ERROR
        0x07,  // SYSTEM_ERROR
        0x08,  // SENDING_ERROR
        0x09,  // MULTIHOPING_ERROR
        0x0A,  // ACTIVATE_REQUEST_BEFORE_ERROR
        0x0B,  // DATA_NOT_EMPTY_STANDBY_ERROR
    ...
    */
     ...
     */
    public static getEnumByReasonCode(reasonCode : number) : NackReason {
        for (var i = 0; i < _NackReason.REASON_CODE_MAP.length; i++) {
            if (_NackReason.REASON_CODE_MAP[i] == reasonCode) {
                return i;
            }
        }

        throw new Error("not found reasonCode: " + reasonCode);
    }
}
