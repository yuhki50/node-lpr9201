...
Sender.prototype.writeProfileParameter = function(paramNo, value) {
  ...
  var dataLengthByteSize = ProfileParameter.DATA_LENGTH_BYTE_SIZE_MAP[paramNo];
  for (var i = dataLengthByteSize - 1; i >= 0; i--) {
    datas.push((value >>> (8 * i)) & 0xFF);
  }

  this.send(Command.WRITE_PROFILE_PARAMETER, datas);
};
...
Sender.prototype.send = function(command, data) {
  ...
  // data length
  if (Command.DATA_LENGTH_BYTE_SIZE_MAP[command] !== undefined) {
    // 指定されたデータ長の上位1byteづつ切り出す {high, mid... low}
    for (var i = Command.DATA_LENGTH_BYTE_SIZE_MAP[command] - 1; i >= 0; i--) {
